function [w_phi, theta_or, phi_x, phi_y, M_proc]=calcSpatialFreqsHilbert2D(g, M, wTh, hilbertFilterDirection, filterFreqsFlag, av_filter_size)
% calcSpatialFreqsHilbert2D(g, M, wTh, hilbertFilterDirection) calculates
% the spatial freq module w_phi, fringe orientation theta_or and its components phi_x and phi_y all in rad/px for
...
arguments
    ...
    av_filter_size (2,1) {mustBeNumeric}=[5 5]; %averaging filter size for spatial freqs
end
...
Nmed=2; % median filter size for phase only cosine-sine filtering
LPCycles=2; %low pass cycles
[phi_x, phi_y, M_proc]=phaseGradient(z, M, NS, Nmed, LPCycles); %rad/px
